package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      public function show() : void
      {
         ...
         this.var_45.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_341));
         Logger.log("BEGINNING TO SHOW: " + this.var_341);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1419;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_341);
         if(param1.flatId != this.var_341)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_341);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_422 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_422.procedure = this.onInputClick;
         this.var_1006 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1006);
         this.var_1006.procedure = this.onSelectTemplate;
         this.var_868 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1296 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1295 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_45.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_45,WindowTracker.const_607,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_533 || this.var_422.text == "")
         {
         ...
         this.var_45.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1391,_loc2_,this.var_422.text,"",""));
         if(this.var_1296.isSelected || this.var_1295.isSelected || this.var_868.isSelected)
         {
            this.var_45.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1296.isSelected,this.var_1295.isSelected,this.var_868.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1280);
         }
      ...
      {
         if(param1.type != WindowEvent.const_180)
         {
            return;
         }
         if(!this.var_533)
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_45.initMsg.roomMessageTemplates[this.var_1006.selection];
         if(_loc3_ != null)
         {
            this.var_533 = false;
            this.var_422.text = _loc3_;
         }
      }
   }
}
